...
Creating a new component model in GUILDA means creating an "m-file" that defines a new child class from the component class. This page explains how to define this child class file.
...
The `component` class defines several functions that should be provided in a newly created component model (e.g., a synchronous generator model) for analysis. By inheriting the properties from the already defined `component` class, newly created component models will automatically have these functions.

However, even though several functions are inherited from the `component` class, still other functions are required to define the dynamic characteristics of the new component model. Such functions are added to the `component` class as abstract methods.
...
_Note: When creating a new component, it is recommended that a description and how to execute the newly define component is included as a comment at the beginning._
...
_Note: When defining the required parameters of the new component, it is recommended to name it as "parameter"_
...
- `Veq`: Equilibrium point of the voltage (complex number) $-$ _obtained from the power flow calculation._

- `Ieq`: Equilibrium point of the current (complex number) $-$ _obtained from the power flow calculation._
...
_Note: To implement this method a linearized state-space model is needed. For it the method "Approximate Linearization" (`get_linear_matrix`) is available._
